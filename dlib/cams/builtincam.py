...
__all__ = ['BuiltinCam', 'SegmentationCam']
...
class BuiltinCam:
    ...
    def assert_model(model: STDClassifier) -> None:
        ...
        assert isinstance(model, STDClassifier)
        ...
        assert any([isinstance(model.classification_head, dlib.poolings.GAP),
                    isinstance(model.classification_head,
...
class SegmentationCam:
    ...
    @staticmethod
    def assert_model(model: UnetFCAM) -> None:

        assert isinstance(model, UnetFCAM), type(model)
