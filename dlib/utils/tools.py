...
def get_device(args):
    ...
    For GPU:
    run `export CUDA_VISIBLE_DEVICES=$cudaid` before running the python script
    to make pytorch see only the device `cudaid` whihc will be referenced 0.
    ...
    if torch.cuda.is_available():
        ...
        try:
            tag = txt[int(args.cudaid)]
        except IndexError:
            tag = ''

        DLLogger.log(message="Device: {} ID: {} - {}".format(
            device, args.cudaid, tag))
    else:
        DLLogger.log(message="Device: {}".format(device))
...
def bye(args):
    ...
    # clean cc
    if is_cc():
        scratch_exp_fd = join(os.environ["SCRATCH"], 'fcam', args.subpath)
