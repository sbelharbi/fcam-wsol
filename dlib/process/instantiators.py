...
def get_loss(args, device):
    masterloss = losses.MasterLoss(device=device)
    ...
    if args.task == constants.STD_CL:
        masterloss.add(losses.ClLoss(
            device=device, support_background=support_background,
    ...
    # fcams
        ...
        elb = ELB(init_t=args.elb_init_t, max_t=args.elb_max_t,
                  mulcoef=args.elb_mulcoef).to(device)

        if args.im_rec:
            masterloss.add(
                losses.ImgReconstruction(
                    device=device, lambda_=args.im_rec_lambda,
        ...
        if args.crf_fc:
            masterloss.add(losses.ConRanFieldFcams(
                device=device, lambda_=args.crf_lambda,
        ...
        if args.entropy_fc:
            masterloss.add(losses.EntropyFcams(
                device=device, lambda_=args.entropy_fc_lambda,
                support_background=support_background,
                multi_label_flag=multi_label_flag))

        if args.max_sizepos_fc:
            masterloss.add(losses.MaxSizePositiveFcams(
                device=device, lambda_=args.max_sizepos_fc_lambda,
        ...
        if args.sl_fc:
            sl_fcam = losses.SelfLearningFcams(
                device=device, lambda_=args.sl_fc_lambda,
                support_background=support_background,
                multi_label_flag=multi_label_flag,
